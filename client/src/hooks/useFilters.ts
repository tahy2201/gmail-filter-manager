...
export function useFilters() {
  ...
  const importFromXml = useCallback(async (xml: string) => {
    setLoading(true);
    setError(null);
    try {
  ...
  const addFilter = useCallback(async (filter: Omit<FilterEntry, 'id'>) => {
  ...
  const updateFilter = useCallback(async (filterId: string, updates: Partial<FilterEntry>) => {
