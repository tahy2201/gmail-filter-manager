...
### 手順
1. スプレッドシートのバックアップを取る（コピーを作成）
2. `clasp push --force` で新コードをデプロイ
3. GASエディタで `migrateDeleteRules()` を実行（DeleteRulesシートを3列→4列に変換）
4. 実行ログを確認し、スキップされたルールがないか確認
5. Webアプリにアクセスして動作確認
...
- Gmailに存在しないラベルの削除ルールはスキップされる（ログに警告が出力される）

### 制約事項
- フィルタ更新時は内部でcreate→deleteが実行される（Gmail APIにupdate操作がないため）
